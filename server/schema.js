...
con.connect(function (err) {
  ...
  var nftTable =
    "CREATE TABLE nft_list (name varchar(45) DEFAULT NULL,token_id varchar(100) NOT NULL, owner_id int DEFAULT 0, nft_ethadr varchar(100) NOT NULL,price_eth int DEFAULT NULL,is_avl int NOT NULL,upd_date datetime DEFAULT NULL,PRIMARY KEY (token_id),CONSTRAINT owner FOREIGN KEY (owner_id) REFERENCES login (login_id));";
  var transTable =
  ...
  con.query(nftInsert1, [privateKey1, d], function (err, result) {
    console.log("Record inserted");
  });

  var nftInsert2 = "INSERT INTO nft_list VALUES ('IronMan',2,100,?,5,1,?)";
  con.query(nftInsert2, [privateKey2, d], function (err, result) {
    console.log("Record inserted");
  });

  var nftInsert3 = "INSERT INTO nft_list VALUES ('GreenPepper',3,100,?,2,1,?)";
  con.query(nftInsert3, [privateKey3, d], function (err, result) {
    console.log("Record inserted");
  });

  var nftInsert4 = "INSERT INTO nft_list VALUES ('WaterNebula',4,100,?,4,1,?)";
  con.query(nftInsert4, [privateKey4, d], function (err, result) {
    console.log("Record inserted");
  });

  var nftInsert5 = "INSERT INTO nft_list VALUES ('SteelThrone',5,100,?,5,1,?)";
  con.query(nftInsert5, [privateKey5, d], function (err, result) {
    console.log("Record inserted");
  });
